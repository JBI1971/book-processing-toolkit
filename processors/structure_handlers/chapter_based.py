#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Chapter-based structure handler (第N章 format)."""
...
class ChapterBasedHandler(BaseStructureHandler):
    """Handler for books using 第N章 (chapter) format."""

    def can_handle(self, json_data: Dict[str, Any]) -> float:
        """
        Check if data uses chapter-based structure.
        ...
        # Count chapters with 第N章 pattern
        chapter_count = 0
        total_count = min(10, len(chapters))  # Sample first 10

        for chapter in chapters[:total_count]:
            title = chapter.get('title', '')
            if re.match(r'第[一二三四五六七八九十廿卅卌百千\d]+章', title):
                chapter_count += 1

        confidence = chapter_count / total_count if total_count > 0 else 0.0
        return confidence

    def discover_structure(self, json_data: Dict[str, Any]) -> StructureDiscoveryResult:
        """
        Discover chapter-based structure.

        Pass 1: Extract all content, identify TOC, chapters, and intro.
        """
        result = StructureDiscoveryResult()
        result.detected_format = "chapter"
