        ...
        if isinstance(toc_data, list):
            for idx, item in enumerate(toc_data):
                if isinstance(item, dict):
                    # Nested structure: [{..., "entries": [...]}, ...]
                    if 'entries' in item:
                        nested_entries = item.get('entries', [])
                        for entry_idx, entry in enumerate(nested_entries):
                            if isinstance(entry, dict) and 'chapter_number' in entry:
                                entries.append(TOCEntry(
                                    toc_index=entry_idx,
                                    full_title=entry.get('full_title', ''),
                                    chapter_title=entry.get('chapter_title', ''),
                                    chapter_number=entry.get('chapter_number', entry_idx + 1),
                                    chapter_id=entry.get('chapter_id', f'chapter_{entry_idx+1:04d}')
                                ))
                    # Flat array (old format): [{chapter_number, ...}, ...]
                    elif 'chapter_number' in item:
                        entries.append(TOCEntry(
                            toc_index=idx,
                            full_title=item.get('full_title', ''),
                            chapter_title=item.get('chapter_title', ''),
                            chapter_number=item.get('chapter_number', idx + 1),
                            chapter_id=item.get('chapter_id', f'chapter_{idx+1:04d}')
                        ))
